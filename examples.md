...
Questions from the English guide. Choose an option first, then open the spoiler to check the answer and explanation.
...
**Correct answer: A) Add a programmatic precondition that blocks `lookup_order` and `process_refund` until an ID is obtained from `get_customer`**
...
**Correct answer: B) Expand each tool’s description with input formats, examples, and boundaries**
...
**Correct answer: A) Add explicit escalation criteria with few-shot examples**
...
**Correct answer: A) `.claude/commands/` in the project repository**
...
**Correct answer: A) Planning mode: explore the codebase, understand dependencies, design an approach**
...
**Correct answer: A) `.claude/rules/` files with YAML frontmatter and glob patterns**
...
**Correct answer: B) The coordinator decomposed the task too narrowly**
...
**Correct answer: A) Return structured error context to the coordinator: failure type, query, partial results, and alternatives**
...
**Correct answer: A) Give the synthesis agent a limited `verify_fact` tool for simple checks, and continue routing complex verification through the coordinator**
...
**Correct answer: A) Use the `-p` flag: `claude -p "Analyze this pull request for security issues"`**
...
**Correct answer: A) Use batch processing only for tech-debt reports; keep real-time calls for pre-merge checks**
...
**Correct answer: A) Split into focused passes: analyze each file individually for local issues, then run a separate integration pass for cross-file data flows**
...
**Correct answer: D) Complete analysis with both numbers, explicitly annotate the conflict with source attribution, and let the coordinator decide how to reconcile the data before passing to synthesis.**
...
**Correct answer: C) The coordinator passes both sets of results to the synthesis agent for a unified integration.**
...
**Correct answer: C) The coordinator’s task decomposition is too narrow, assigning subagents work that does not cover all relevant areas.**
...
**Correct answer: D) Structure the synthesis output with coverage annotations that indicate which conclusions are well-supported and where gaps exist due to unavailable sources.**
...
**Correct answer: A) Return an error with context to the coordinator agent, allowing it to decide how to proceed.**
...
**Correct answer: A) The coordinator can observe all interactions, handle errors uniformly, and decide what information each subagent should receive.**
...
**Correct answer: A) Return structured error context to the coordinator including the failure type, the query executed, any partial results, and potential alternative approaches.**
...
**Correct answer: B) The coordinator explicitly partitions the research space before delegating, assigning each agent distinct subtopics or source types.**
...
**Correct answer: C) Place a key-findings summary at the start of the aggregated input and organize detailed results with explicit section headings for easier navigation.**
...
**Correct answer: A) Modify upstream agents to return structured data (key facts, quotes, relevance scores) instead of verbose content and reasoning.**
...
**Correct answer: D) Give the synthesis agent a limited-scope `verify_fact` tool for simple checks, while routing complex verifications through the coordinator to the web-search agent.**
...
**Correct answer: B) Use the CLI flags `--output-format json` and `--json-schema` to enforce structured findings, then parse the output to post inline comments via the GitHub API.**
...
**Correct answer: A) Run a second independent instance of Claude Code to review the changes without access to the generator’s reasoning.**
...
**Correct answer: D) Add 3–4 few-shot examples showing the exact required format: identified issue, location in code, concrete fix suggestion.**
...
**Correct answer: B) Only the deep analysis.**
...
**Correct answer: A) Temporarily disable high-false-positive categories (style, naming, documentation) and keep only high-precision categories while improving prompts.**
...
**Correct answer: A) Include the existing test file in context so Claude can determine what scenarios are already covered.**
...
**Correct answer: D) Include previous review findings in context and instruct Claude to report only new or still-unresolved issues.**
...
**Correct answer: B) Add the `-p` flag: `claude -p "Analyze this pull request for security issues"`.**
...
**Correct answer: B) Split into focused passes: review each file individually for local issues, then run a separate integration-oriented pass to examine cross-file data flows.**
...
**Correct answer: C) Use batch processing only for technical debt reports; keep synchronous calls for pre-merge checks.**
...
**Correct answer: B) Provide 2–3 concrete input-output examples showing the expected transformation for representative API responses.**
...
**Correct answer: B) Switch to planning mode to explore integration options and architectural implications, then present a recommendation before implementation.**
...
**Correct answer: A) Switch to planning mode to explore the codebase, understand dependencies, and design the implementation approach before making changes.**
...
**Correct answer: A) Add `context: fork` in the skill frontmatter to run the analysis in an isolated subagent context.**
...
**Correct answer: A) Create a personal version under `~/.claude/skills/` with a different name, e.g., `/my-commit`.**
...
**Correct answer: A) The guidance lives in the original developers’ user-level `~/.claude/CLAUDE.md` files, not in the project `.claude/CLAUDE.md`. Move the instruction to the project-level file so all team members receive it.**
...
**Correct answer: B) Add `argument-hint` in frontmatter to request required parameters, use `context: fork` to isolate execution, and restrict `allowed-tools` to file-write operations.**
...
**Correct answer: D) Create rule files under `.claude/rules/` with YAML frontmatter specifying glob patterns to conditionally apply conventions based on file paths.**
...
**Correct answer: B) In the project repository under `.claude/commands/`.**
...
**Correct answer: B) Add `context: fork` in the skill frontmatter.**
...
**Correct answer: A) Use an Explore subagent for Phase 1 to isolate verbose discovery output and return a summary, then continue Phases 2–3 in the main conversation.**
...
**Correct answer: D) Check the tool descriptions to ensure they clearly differentiate each tool’s purpose.**
...
**Correct answer: C) Add few-shot examples to the prompt demonstrating correct reasoning and tool sequencing for multi-issue requests.**
...
**Correct answer: C) Decompose the request into separate issues, then investigate each in parallel using shared customer context before synthesizing a final resolution.**
...
**Correct answer: C) Add explicit escalation criteria to the system prompt with few-shot examples showing when to escalate versus resolve autonomously.**
...
**Correct answer: C) A customer requests competitor price matching. Your policies allow price adjustments for price drops on your own site within 14 days, but say nothing about competitor prices. The agent should escalate for policy interpretation.**
...
**Correct answer: C) Add a programmatic precondition that blocks `lookup_order` and `process_refund` until `get_customer` returns a verified customer identifier.**
...
**Correct answer: A) Add a self-critique stage where the agent evaluates a draft response for completeness—ensuring it resolves the customer’s issue, includes relevant context, and anticipates follow-up questions.**
...
**Correct answer: D) Instruct Claude in the prompt to bundle tool requests into one turn and return all results together before the next API call.**
...
**Correct answer: C) Extract transactional facts (amounts, dates, order numbers) into a persistent “case facts” block included in every prompt outside the summarized history.**
...
**Correct answer: B) Instruct Claude to request an additional identifier (email, phone, or order number) when `get_customer` returns multiple matches before taking any customer-specific action.**
...
**Correct answer: D) Expand each tool’s description to include input formats, example queries, edge cases, and boundaries explaining when to use it versus similar tools.**
...
**Correct answer: A) Check the `stop_reason` field in Claude’s response—continue if it is `tool_use` and stop if it is `end_turn`.**
...
**Correct answer: A) Use a PostToolUse hook to intercept tool outputs and apply formatting transformations before the agent processes them.**
...
**Correct answer: C) Add 4–6 examples targeted at ambiguous scenarios, each with rationale for why one tool was chosen over plausible alternatives.**
...
**Correct answer: D) Replace with two tools: `preview_remove_member` returns impact details and a single-use confirmation token; `execute_remove_member` requires that token, binding execution to the preview.**
...
**Correct answer: C) Implement automatic retry with backoff for network timeouts inside the tool; return syntax errors immediately with parameter validation details.**
...
**Correct answer: A) Surface the contradiction and ask the user to clarify which matters more.**
...
**Correct answer: D) Your application isn't including prior messages in the `messages` array.**
...
**Correct answer: C) Extract critical structured data (allergies, quantities, preferences), summarize general discussion, and keep recent exchanges verbatim.**
...
**Correct answer: A) Hybrid approach: summarize older messages while keeping recent ones verbatim.**
...
**Correct answer: A) The entire conversation history is included with each API request.**
...
**Correct answer: C) Semantic embeddings with retrieval of relevant exchanges.**
...
**Correct answer: C) Insert user-role messages reinforcing guidelines at conversation breakpoints.**
...
**Correct answer: B) Replace verbose rules with few-shot examples demonstrating proficiency-level adaptation.**
...
**Correct answer: B) In the system prompt.**
...
**Correct answer: A) Append a partial assistant message with a direct response opening.**
...
**Correct answer: D) Append the status update as a prefix to the next user message.**
...
**Correct answer: C) State assumptions explicitly and proceed while inviting corrections.**
...
**Correct answer: C) Accumulated assistant responses dilute system prompt influence.**
...
**Correct answer: A) Make reasonable assumptions, state them explicitly, and offer to adjust.**
